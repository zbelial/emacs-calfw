...
(defcustom cfw:reference-size-fraction 1.0
  "If not specified, the width/height of the reference size will be
shrinked to this portion of the window.
Only used in `cfw:dest-init-buffer' ATM."
  :group 'cfw
  :type 'float)
  ...
  (let
    ...
    (setq dest
          (make-cfw:dest
           ...
           :max-func 'point-max
           :buffer buffer
           :width (or width (round (* (window-width window) cfw:reference-size-fraction)))
           :height (or height (round (* (window-height window) cfw:reference-size-fraction)))
           :clear-func (lambda ()
...
(defun cfw:refresh-calendar-buffer (resize)
  "Clear the calendar and render again.
With prefix arg RESIZE, fit calendar to window size."
  (interactive "P")
  (let ((cp (cfw:cp-get-component)))
    (when cp
      (when resize
